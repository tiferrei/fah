...
CMD ["sh", "-c", "/usr/bin/FAHClient --pid-file=/var/run/fahclient.pid --user=\"${USER}\" --team=\"${TEAM}\" --passkey=\"${PASSKEY}\" --power=full --gui-enabled=false --gpu=true --smp=true --web-allow=\"0/0\" --allow=\"0/0\" --command-allow-no-pass=\"0/0\""]
